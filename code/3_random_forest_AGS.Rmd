...
AGS_vfold <- vfold_cv(RF_train, v = 5, repeats = 5) # usually min of 5-fold cross-validation
...
AGS_recipe <- 
  ...
  step_rm(ID, horizon, siteID, soilID, plotID, collectDate, year, X,
          ...
          GC_bact, GC_marker, Coding_density_marker, num_taxa_marker, AGS_marker, 
...
rf_prepped <- prep(AGS_recipe, training = RF_train) # preps data, applies recipe
...
saveRDS(rf_fit_results, "../data/input/random_forest/AGSBact_model_RF_full.rds")

rf_fit_results <- readRDS("../data/input/random_forest/AGSBact_model_RF_full.rds")
...
rf_fit_update <- tune_grid(AGS_wflow, 
                           resamples = AGS_vfold,
                           grid = rf_grid_update,
                           metrics = metric_set(rmse, mae, rsq))

saveRDS(rf_fit_update, "../data/input/random_forest/AGSBact_model_RF_small.rds")

rf_fit_update<-readRDS("../data/input/random_forest/AGSBact_model_RF_small.rds")
...
final_res %>% 
  collect_predictions(summarize = FALSE) %>% arrange(desc(AGS))
...
rf_plot <- final_res %>%
  ...
  labs(y = 'Random Forest Predicted AGS', x = 'Original AGS') +
...
for_cor_matrix <- rf_df %>% select(-c(ID, horizon, siteID, soilID, plotID, collectDate, year, X,
          dnaSampleID, verticalPosition, soilID, sampleTotalReadNumber,
          sampleFilteredReadNumber, reads, plotID, test, GC, Genome_C, Genome_CN, Genome_N, 
          year, Genomes, Nuc_CN, AA_CN, GC_bact, bp, GC_adj, GC_bact))%>% 
...
Fig_3_maybe <- rf_vip_train+
  theme_pete()+
  geom_point(size = 4, shape = 21, fill = "darkred")
Fig_3_maybe

ggsave(plot = Fig_3_maybe, filename = "../paper/figures/Fig_3_maybe.png")
```
